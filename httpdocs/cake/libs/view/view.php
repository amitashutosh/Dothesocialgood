 ...
 */
	function element($name, $params = array(), $loadHelpers = false) {
		...
		/**
		 * Changed by Thaissa Mendes
		 * To fix bug when calling an element from a plugin
		*/
		...
		foreach ($exts as $ext) {
			foreach ($paths as $path) {
				/**
				* Changed by Thaissa Mendes
				* To fix bug when calling an element using relative path
				*/

				if (strpos($name, '../') === 0)
				{
					$_name = str_replace('../','',$name);
					if (file_exists($path . $_name . $this->ext)) {
						$file = $path . $_name . $this->ext;
						break;
					}
				}
				else
 ...
 */
	function _getViewFileName($name = null) {
		...
		if (strpos($name, DS) === false && $name[0] !== '.') {
			 ...
			 * @since dec 11 2010
			 */
			$name = (isset($this->params['prefix']) ? $this->params['prefix'].DS : '') . $this->viewPath . DS . $subDir . Inflector::underscore($name);
