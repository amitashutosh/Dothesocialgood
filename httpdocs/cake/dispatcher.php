	...
	function dispatch($url = null, $additionalParams = array()) {
		...
		if (!empty($prefixes)) {
			if (isset($this->params['prefix'])) {
				// Modification by Thaissa Mendes - removing prefix of params action
 ...
 */
	function __loadController($params) {
		...
		// extra by Thaissa Mendes
		// separate controllers in folder by prefix and plugin
		if (!empty($params["prefix"]))
		{
			// importing AdminController
			App::import('Controller', Inflector::camelize($params["prefix"]), true, array(APP . 'controllers' . DS . $params["prefix"] . DS));

			// if a plugin controller is being called
			if (!empty($this->params['plugin']))
			{
				// import controller inside plugin/controller/admin folder
				require_once(APP . 'plugins' . DS . $params['plugin'] . DS . 'controllers' . DS . $params["prefix"] . DS . Inflector::tableize($controller) . '_controller.php');
				return $controller;
			}
			else
			{
				// import controller inside controller/admin folder
				if (App::import('Controller', $controller, true, array(APP . 'controllers' . DS . $params["prefix"] . DS)))
				{
					return $controller;
				}
			}
		...
		else
		{
			...
			// controllers not inside plugins
			else
			{
